...
echo.
 echo --------------------------------------------------------------------------------------
 echo Test Environment: %BRAND% %TEST_ENV% %LAN%    %BASE_URL%
 echo --------------------------------------------------------------------------------------
 echo.

 echo Removing previous test results...
 IF exist %PROJECT_DIR%\%TEST_RESULTS%\%BRAND%_%TEST_ENV%_%PLATFORM%_%LAN%_%TEST_RESULTS%  DEL /S /Q /F %PROJECT_DIR%\%TEST_RESULTS%\%BRAND%_%TEST_ENV%_%PLATFORM%_%LAN%_%TEST_RESULTS% | echo > NUL
 echo Done
 echo.
...
REM CALL mvn verify -P %DEFAULTPROFILE% -Dlan=%LAN% -Dbrand=%BRAND% -Dmetafilter=%META_FILTER%  -Dwebdriver.driver=%BROWSER% -Dwebdriver.base.url="%BASE_URL%" -Dplatform=%PLATFORM%
CALL mvn verify -P %DEFAULTPROFILE% -DproxySet=true -DproxyHost=proxy4.wipro.com -DproxyPort=8080 -DproxyUser=PA257736 -DproxyPass=Fossil#in -Dlan=%LAN% -Dbrand=%BRAND% -Dmetafilter=%META_FILTER% -Dplatform=%PLATFORM% -Dbrowserstack.os=%BROWSERSTACK_OS% -Dbrowserstack.os.version=%BROWSERSTACK_OS_VERSION% -Dbrowserstack.browser=%BROWSER% -Dbrowserstack.browser.version=%BROWSER_VERSION% -Dbrowserstack.build=BROWSERSTACK_BUILD -Dbrowserstack.url="%BROWSERSTACK_URL%" -Dwebdriver.base.url="%BASE_URL%"
 ...
 XCOPY /E "%PROJECT_DIR%\target\*.*" "%PROJECT_DIR%\%TEST_RESULTS%\%BRAND%_%TEST_ENV%_%PLATFORM%_%LAN%_%TEST_RESULTS%\*.*"  | echo > NUL
 echo Done
 echo.
